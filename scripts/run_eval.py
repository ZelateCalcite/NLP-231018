...
from eval_scripts.generation_metrics import nlgeval_metrics, usl_score, calc_hfbleu_scores, \
    calc_rouge_scores
...
nlg_ref_metrics = [
    nlgeval_metrics, calc_hfbleu_scores, calc_rouge_scores,
]
...
classification_metrics_list = [classification_report, accuracy_score, accuracy_metric, precision_score, recall_score,
                               f1_score]
...
task_metrics_map = {
    ...
    'eval_binary': [correlation_metrics],
        ...
        if metric in [classification_report, accuracy_score, accuracy_metric, precision_score, recall_score, f1_score]:
            ...
            for dp in tqdm(task_data[:]):
                ...
                if 'rst' in task:
                    ...
                    if len(prediction) > 1 and label != prediction and 'not' not in label:
...
def calculate_metrics(args):
    ...
    for task in task_data_dict.keys():
        ...
        for i, dp in enumerate(task_data_dict[task]):
            ...
            if 'metadata' in dp and 'eval_dataset_name' in dp['metadata'] and dp['metadata'][
                'eval_dataset_name'] is not None:
        ...
        for dataset_name in task_datasetspecific_dict.keys():
            ...
            metrics_dict[task]['dataset_metrics'][dataset_name]['num_datapoints'] = len(
                task_datasetspecific_dict[dataset_name])
            calculate_task_metrics(task, task_datasetspecific_dict[dataset_name],
                                   metrics_dict[task]['dataset_metrics'][dataset_name], dataset_level=True)
        ...
        if args.instructionmetrics:
            ...
            for inst_name in task_instruction_specific_dict.keys():
                ...
                metrics_dict[task]['instruction_metrics'][inst_name]['num_datapoints'] = len(
                    task_instruction_specific_dict[inst_name])
                calculate_task_metrics(task, task_instruction_specific_dict[inst_name],
                                       metrics_dict[task]['instruction_metrics'][inst_name], instruction_level=True)
